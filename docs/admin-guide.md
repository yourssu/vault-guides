...
1. **+ 새 그룹** 버튼 클릭
2. 그룹 이름 입력 (예: `backend`, `design`, `iOS`)
3. 접근을 허용할 **컬렉션** 선택
4. 저장
...
| 리드 (`head-leads`, `part-leads`, `dev-leads`) | **Manage collection** | 컬렉션 구성원 권한까지 관리 가능해야 함 |
| `commons` 컬렉션 전체 구성원 | **Edit items** | 공용 계정은 누구나 추가·수정 가능, 멤버 관리는 관리자만 |
...
|--------|----------|
| `teams` | 전체 파트 그룹 |
| `leads` | `head-leads`, `part-leads`, `dev-leads` |
| `commons` | 전체 구성원 |
| 파트별 컬렉션 (예: `backend`, `design`, `iOS` 등) | 해당 파트 그룹 |
