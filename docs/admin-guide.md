...
![구성원 목록](images/admin-members.png)
...
|----------|------|------|
| `org` | 조직 전체 | `org:urssu` |
| `team` | 파트/팀 소속 | `team:fe`, `team:ios`, `team:design` |
| `role` | 역할 또는 서비스/프로젝트 단위 권한 | `role:dev-lead`, `role:soongpt`, `role:signal` |

**케이스별 예시:**

| 케이스 | 그룹 이름 | 설명 |
|--------|-----------|------|
| 새 팀 추가 | `team:backend` | 팀 이름을 value로 사용 |
| 리드 권한 부여 | `role:part-lead` | 역할 이름을 value로 사용 |
| 서비스별 접근 제한 | `role:soongpt` | 특정 서비스에 접근하는 구성원 묶음 |
| 채용 담당자 권한 | `role:recruting-manager` | 특정 업무 권한이 필요한 경우 |

### 새 그룹 만들기

1. **+ 새 그룹** 버튼 클릭
2. 그룹 이름 입력 — 네이밍 정책에 따라 `{category}:{value}` 형식으로 작성
3. 접근을 허용할 **컬렉션** 선택
4. 저장
...
컬렉션 이름은 `{scope}:{type}` 형식으로 지정합니다. 모두 소문자, 단어 구분은 `-`(하이픈)을 사용합니다.

**스코프 종류:**

| 스코프 | 대상 | 허용 type |
|--------|------|-----------|
| `android` | 안드로이드 팀 | `account`, `config` |
| `ios` | iOS 팀 | `account`, `config` |
| `web` | 프론트엔드 팀 | `account`, `config` |
| `infra` | 인프라 담당자 | `account`, `config` |
| `tool` | 비개발팀 | 팀 이름 (예: `tool:hr`) |
| `shared` | 전체 구성원 | `common` |

**type 구분 기준 (`account` / `config`):**

- **`account`** — 사람이 직접 로그인하는 계정 (username/password, Play Console, App Store Connect, SSH 키 등)
- **`config`** — 코드·시스템에서 사용하는 설정값 (API 키, 설정 파일, 접속 정보, 환경변수)

**케이스별 예시:**

| 케이스 | 컬렉션 | 설명 |
|--------|--------|------|
| 안드로이드 앱 서명 키, Play Console 계정 | `android:account` | 사람이 로그인하는 계정·서명 자격증명 |
| google-services.json, Firebase API 키 | `android:config` | 앱 빌드에 필요한 설정 파일 |
| App Store Connect 계정, 인증서 | `ios:account` | iOS 배포용 계정·인증서 |
| 서버 SSH 접속, 클라우드 콘솔 계정 | `infra:account` | 인프라 접근용 로그인 자격증명 |
| DB 접속 정보, 서버 환경변수 | `infra:config` | 서버·앱 구동에 필요한 설정값 |
| 마케팅팀 SNS, SaaS 도구 계정 | `tool:marketing` | 팀 이름을 type으로 사용 |
| Zapier, Make 등 업무 자동화 | `tool:workflow` | 특정 팀에 귀속되지 않는 업무 자동화 |
| 전사 공용 계정 | `shared:common` | 모든 구성원이 접근하는 공용 계정 |

컬렉션은 고정 운영합니다. 새 컬렉션이 필요한 경우 관리자에게 문의하세요.

### 새 컬렉션 만들기

1. 우측 상단 **+ 새 컬렉션** 클릭
2. 컬렉션 이름 입력 — 네이밍 정책에 따라 `{scope}:{type}` 형식으로 작성
...
| 컬렉션 | 접근 그룹 |
|--------|----------|
| `android:account`, `android:config` | `team:android` |
| `ios:account`, `ios:config` | `team:ios` |
| `web:account`, `web:config` | `team:fe` |
| `infra:account`, `infra:config` | infra 담당자 |
...
확장 프로그램 또는 앱이 `bitwarden.com` 공식 서버를 바라보고 있을 가능성이 높습니다. 서버 URL을 `https://vault.yourssu.com` 으로 설정하도록 안내하세요. ([일반 사용자 가이드 참고](user-guide.md))
