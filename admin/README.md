...
`admin/` 디렉토리는 [git-crypt](https://github.com/AGWA/git-crypt)와 GPG를 사용해 암호화됩니다.
...
GPG 키는 **인프라(정), 인프라(부) 최대 2명**만 보유합니다. 불필요한 키 등록은 금지합니다.

서버 어드민 키(`/admin 로그인 비밀번호`)는 `admin/src/.admin-credentials`에 위치합니다.
...
unlock 후에는 `admin/` 파일이 평문으로 보입니다.

---

## 5. 새 파일 추가

`admin/` 안에 파일을 추가하면 `.gitattributes` 설정에 의해 자동으로 암호화됩니다.
...
lock 후에는 `admin/` 파일이 다시 바이너리(암호화)로 표시됩니다.
...
| 파일 | 역할 |
|------|------|
| `.gitattributes` | 암호화 대상 경로 지정 (`admin/**`) |
